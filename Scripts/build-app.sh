#!/bin/bash
# Builds MicroduckDesktop.app into build/.
...
# shellcheck disable=SC2086
BIN="$(swift build -c release $ARCH_FLAGS --show-bin-path)/MicroduckDesktop"
APP="build/MicroduckDesktop.app"

echo "==> Assembling $APP"
rm -rf "$APP"
mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources"
cp "$BIN" "$APP/Contents/MacOS/MicroduckDesktop"
cp Support/Info.plist "$APP/Contents/Info.plist"

echo "==> Generating icon"
Scripts/make-icon.sh "$APP/Contents/MacOS/MicroduckDesktop" "$APP/Contents/Resources/AppIcon.icns"
